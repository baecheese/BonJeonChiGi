//
//  TimeExtenstion.swift
//  BonJeonChiGi
//
//  Created by 배지영 on 2017. 7. 20..
...
extension TimeInterval {
    ...
    func getHHMM(date:Date) -> String {
        return formatString(date: date, format: "HH:mm")
    }
    
    func getDotDate(date:Date) -> String {
        return formatString(date: date, format: "yyyy.MM.dd")
    }
    
    func getYYMMDD(date:Date) -> String {
        return formatString(date: date, format: "yyyy-MM-dd")
    }
    
    func formatString(date:Date, format:String) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format
        dateFormatter.locale = NSLocale.current
        return dateFormatter.string(from: date as Date)
